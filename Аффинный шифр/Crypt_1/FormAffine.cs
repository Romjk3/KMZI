...
namespace Crypt_1
{
    public partial class FormAffine : Form
    {
        ...
        private static FormAffine f;
        public static FormAffine fd
        {
            get
            {
                if (f == null || f.IsDisposed) f = new FormAffine();
                return f;
            }
        }
        ...
        private void buttonCrypt_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                int Alpha = int.Parse(textBoxA.Text);
                int Beta = int.Parse(textBoxB.Text);
                if ((Alpha % 13 == 0) || (Alpha % 2 == 0))
                {
                    throw new Exception();
                }
        ...
        private void buttonUncrypt_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                int Beta = int.Parse(textBoxB.Text);
                int j = 0;
                    while ((26 * j + 1) % Alpha != 0) 
                    {
                        j++;
                    }
                    int Alpha1 = (26 * j + 1) / Alpha;
                if ((Alpha % 13 == 0) || (Alpha % 2 == 0))
                {
                    throw new Exception();
                }
